...
| `cv-template.md`, `cv-template.long-dang.md` | `build-cv-markdown.mjs` (via `cv-templates.mjs`) | Markdown CV templates. Headings and order come from the template; inline HTML for generated contact and project blocks comes from the shared renderer and is preserved in output. |
