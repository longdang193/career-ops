...
export const ENTRY_FIELD_SPECS = {
  html: {
    experience: {
      ...
      known: ['company', 'role', 'location', 'dates', 'period', 'description', 'bullets'],
    },
    projects: {
      required: ['name'],
      known: ['name', 'badge', 'description', 'bullets', 'tech', 'methodology', 'domain', 'dates', 'period', 'url'],
    },
    education: {
      required: ['title'],
      known: ['title', 'org', 'location', 'year', 'description', 'coursework', 'subjects', 'gpa'],
    },
    certifications: { required: ['title'], known: ['title', 'org', 'year', 'url', 'focus', 'description'] },
  ...
  tex: {
    ...
    projects: {
      required: ['name'],
      known: ['name', 'context', 'tech', 'methodology', 'domain', 'dates', 'bullets', 'url'],
    },
    education: {
...
export const MAX_CV_ENTRY_BULLETS = 4;
export const MAX_CV_CERTIFICATIONS = 5;
export const MIN_CERTIFICATE_FOCUS = 2;
export const MAX_CERTIFICATE_FOCUS = 5;
export const MAX_CV_PROJECTS = 3;
export const MAX_EDUCATION_SUBJECTS = 5;

function normalizeEducationEntry(entry) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return entry;
  const normalized = { ...entry };
  if (typeof normalized.location === 'string') {
    normalized.location = normalized.location.split(',').map(value => value.trim()).filter(Boolean).at(-1) || normalized.location;
  }
  for (const key of ['coursework', 'subjects']) {
    if (Array.isArray(normalized[key])) normalized[key] = normalized[key].slice(0, MAX_EDUCATION_SUBJECTS).join('; ');
    else if (typeof normalized[key] === 'string') {
      const subjects = normalized[key].split(/\s*;\s*|\r?\n/).map(value => value.trim()).filter(Boolean);
      if (subjects.length > MAX_EDUCATION_SUBJECTS) normalized[key] = subjects.slice(0, MAX_EDUCATION_SUBJECTS).join('; ');
    }
  }
  return normalized;
}

export function normalizeCvPayload(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return payload;

  const normalized = { ...payload };
  for (const section of ['experience', 'projects']) {
    if (!Array.isArray(payload[section])) continue;
    normalized[section] = payload[section].slice(0, section === 'projects' ? MAX_CV_PROJECTS : undefined).map((entry) => {
      if (!entry || typeof entry !== 'object' || Array.isArray(entry) || !Array.isArray(entry.bullets)) {
        if (section === 'projects' && entry && typeof entry === 'object' && !entry.tech) {
          return { ...entry, tech: entry.methodology || entry.domain || '' };
        }
        return entry;
      }
      return {
        ...entry,
        bullets: entry.bullets.slice(0, MAX_CV_ENTRY_BULLETS),
        ...(section === 'projects' && !entry.tech ? { tech: entry.methodology || entry.domain || '' } : {}),
      };
    });
  }
  if (Array.isArray(payload.education)) normalized.education = payload.education.map(normalizeEducationEntry);
  if (Array.isArray(payload.certifications)) {
    normalized.certifications = payload.certifications.slice(0, MAX_CV_CERTIFICATIONS).map((entry) => {
      if (!entry || typeof entry !== 'object' || Array.isArray(entry) || !Array.isArray(entry.focus)) return entry;
      return { ...entry, focus: entry.focus.slice(0, MAX_CERTIFICATE_FOCUS).map(value => typeof value === 'string' ? value.trim() : value) };
    });
  }
  return normalized;
}
