...
import { loadDocumentRules } from "../../../lib/document-rules.mjs";

const CV_MIN_WORDS = loadDocumentRules().cv.minWords;
...
test("writeCvHtml: writes the html verbatim", () => {
  ...
  const dir = makeScratchDir();
  const html = '<!DOCTYPE html>\n<html><head><style>a>b{content:"<<"}</style></head><body>José — 5 &lt; 10 ' + 'word '.repeat(CV_MIN_WORDS) + '</body></html>';
...
test("writeCvHtml: rejects a CV outside shared word limits before writing", () => {
  ...
  try {
    ...
    assert.equal(result.ok, false);
    assert.match(result.error, new RegExp(`minimum is ${CV_MIN_WORDS}`));
...
test("writeCvHtml: a shorter re-render leaves no trailing bytes", () => {
  ...
  const dir = makeScratchDir();
  const paths = { html: join(dir, "cv-web-018.html"), finalPdf: join(dir, "out.pdf") };
  const long = `<!DOCTYPE html><html><body>${"long ".repeat(CV_MIN_WORDS + 100)}</body></html>`;
  const short = `<!DOCTYPE html><html><body>${"short ".repeat(CV_MIN_WORDS)}</body></html>`;
  try {
